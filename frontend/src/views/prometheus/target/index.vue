<template>
    <d2-container>
        <template slot="header">Prometheus - Target管理</template>
                ...
                <div v-for="col in tableColumns">
                    <el-table-column :prop="col.key" :label="col.title" v-if="col.key==='Node'">
                        <template slot-scope="scope">
                            <div v-html="scope.row.Job.Node.Hostname"></div>
                        </template>
                    </el-table-column>
                    <el-table-column :prop="col.key" :label="col.title" v-else-if="col.key==='Job'">
                        <template slot-scope="scope">
                            <div v-html="scope.row[col.key].Key"></div>
                        </template>
                    </el-table-column>
            ...
            <el-form ref="form" :model="form" :rules="rules" label-width="120px">
                <el-form-item label="Job" prop="Job">
                    <el-select v-model="form.Job">
                        <el-option v-for="item in jobSelectOptions" :value="item.ID" :key="item.ID" :label="item.Key"></el-option>
                    </el-select>
                </el-form-item>
    ...
    export default {
        ...
        methods: {
            ...
            async listTable() {
                ...
                const res = await this.$api.LIST_PROMETHEUS_TARGET(data)
                this.tableData = res.tableData
                this.tableColumns = res.tableColumns
                this.pagination.total = res.tableTotal
                this.jobSelectOptions = res.jobSelectOptions
                this.loading = false
            },
            ...
            saveForm() {
                this.$refs["form"].validate(async (valid) => {
                    if (valid) {
                        if (this.action === 'add') {
                            await this.$api.ADD_PROMETHEUS_TARGET(this.form)
                            this.listTable()
                        } else if (this.action === 'modify') {
                            const res = await this.$api.MODIFY_PROMETHEUS_TARGET(this.form)
                            // this.notice("恭喜", "修改成功")
                            this.listTable()
                        }
            ...
            modify(row) {
                ...
                this.form.Job = this.form.Job.ID
                this.dialogVisible = true
            },
            ...
            async del(row) {
                ...
                await this.$api.DEL_PROMETHEUS_TARGET(data)
                this.$message({
                    type: 'success',
                    message: '删除成功!'
                });
            ...
            delConfirm(row) {
                this.$confirm('此操作将删除Target, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
